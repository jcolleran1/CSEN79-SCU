...
namespace coen79_lab8
{
    ...
    void deque<Item>::operator=(const deque<Item>& source) {
        ...
        value_type** new_block_pointers = new value_type* [bp_array_size];
        ...
        for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size; ++bp_array_index)
        {
            ...
            else
            {
                ...
                {
                    new_block_pointers[block_item_index] = source.block_pointers[block_item_index];
                }
            }
        }
    }
    ...
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        if (first_bp == NULL)
        {
            ...
            front_ptr = new value_type[block_size];
            back_ptr = front_ptr;
            *first_bp = front_ptr;
            *front_ptr = entry;
        }
        ...
        {
            value_type *temp = new value_type[++bp_array_size];
            *temp = *new value_type[block_size];
            first_bp--;
            *first_bp = temp;
            **first_bp = *front_ptr;
            *front_ptr = entry;
    ...
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        {
            assert(bp_array_size > 1);
            size_t mid = floor(bp_array_size/2); // Get the mid point of the array of block pointers
            
            last_bp = first_bp = block_pointers + mid  - 1;
            front_ptr = new value_type[block_size];
            back_ptr = front_ptr;
            *first_bp = front_ptr;
            *front_ptr = entry;
    ...
    void deque<Item>::pop_front()
    {
        ...
        {
            size_type dif = first_bp - block_pointers;
            delete[] block_pointers[dif];
            first_bp++;
            front_ptr = *first_bp;
